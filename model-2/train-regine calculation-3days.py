...
# 1.3 边界文件与未裁剪的原始 DEM TIF (供 MOE 门控网络使用)
...
def main():
    print("=" * 70)
    print("正在对齐资产 1 空间网格，准备 ST-UNet 数据集...")
    ...
    # 6.2 载入未裁剪 DEM TIF 并重采样对齐全矩形网格空间
    print(f"正在载入未裁剪 DEM 文件 ({os.path.basename(dem_tif)}) 并对齐至全矩形空间...")
    ...
    # 6.8 📊 对齐盲测站点详细指标复盘 (2020-2024 测试集)
    valid_dates = gpm_date_strs[1:-1] 
    test_date_indices = np.where(valid_dates >= '2020-01-01')[0]
    
    print("\n" + "=" * 75)
    print("📊【资产 2 ST-UNet 时空非平稳模型】盲测站点详细指标复盘 (2020-2024)")
    print("=" * 75)
    ...
    # 6.9 导出未裁剪的全矩形 NetCDF 资产 2 (全阵列无 NaN，供 MOE 门控网络直接训练)
    ds_asset2 = xr.Dataset(
        ...
        attrs=dict(
            description="北江中上游基于 ST-UNet 结合 ERA5 与风场水汽的时空非平稳融合降水资产2 (全矩形网格未裁剪版，供 MOE 门控网络训练)",
