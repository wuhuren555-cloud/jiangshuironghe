...
# =========================================================================
# 🏗️ 3. 构建 ST-UNet 深度神经网络 (5天窗口 35 通道输入)
# =========================================================================
...
class STUNet(nn.Module):
    def __init__(self, in_channels=35, out_channels=1): # 🌟 默认改为 35 通道输入
        super().__init__()
...
class SpatiotemporalPrecipDataset(Dataset):
    ...
    def __len__(self):
        # 🌟 5 天窗口：前后各扣除 2 天边缘，有效天数为 N - 4
        return self.num_days - 4

    def __getitem__(self, idx):
        # 🌟 中心时刻 tOffset 为 idx + 2
        t = idx + 2
        f_prev2 = self.daily_features_norm[t-2] # t-2 天
        f_prev1 = self.daily_features_norm[t-1] # t-1 天
        f_curr  = self.daily_features_norm[t]   # t 天 (目标日)
        f_next1 = self.daily_features_norm[t+1] # t+1 天
        f_next2 = self.daily_features_norm[t+2] # t+2 天
        
        # 拼接连续 5 天特征 ➔ 35 通道
        x_5days = torch.cat([f_prev2, f_prev1, f_curr, f_next1, f_next2], dim=0) # (35, H, W)
        y_curr  = self.target[t] # (1, H, W)
...
def main():
    print("=" * 70)
    print("正在对齐资产 1 空间网格，准备 ST-UNet 数据集 (5天滑动窗口 / 35通道)...")
    ...
    # 6.5 三段式划分数据集 (训练集 2012-2017 | 验证集 2018-2019 | 独立测试集 2020-2024)
    ...
    # 6.8 📊 对齐 5 天窗口切片，复盘盲测站点 (2020-2024)
    valid_dates = gpm_date_strs[2:-2] # 🌟 扣除前后 2 天边缘窗口
    test_date_indices = np.where(valid_dates >= '2020-01-01')[0]
    
    print("\n" + "=" * 75)
    print("📊【资产 2 ST-UNet 时空非平稳模型 (5天窗口)】盲测站点详细指标复盘 (2020-2024)")
    print("=" * 75)
    ...
    ds_asset2 = xr.Dataset(
        ...
        coords=dict(
            time=times[2:-2], # 🌟 扣除前后 2 天窗口
            lat=lats,
            lon=lons,
        ),
        attrs=dict(
            description="北江中上游基于 ST-UNet(5天窗口) 结合 ERA5 与风场水汽的时空非平稳融合降水资产2",
    ...
    print("=" * 70)
    print(f"🎉 🎉 🎉 5天窗口版核心资产 2 构建完工！已成功导出！")
